...
import { validateRow as validateRowUtil, titleCase, normalizePhoneSimple, trimString } from '../utils/validation';
...
export const useValidationStore = create<ValidationState>((set, get) => ({
  ...
  validateAll(rows) {
    ...
    rows.forEach((row, idx) => {
      const res = validateRowUtil(row);
      if (!res.ok) {
        errors[idx] = res.errors;
  ...
  validateRow(rows, idx) {
    ...
    set((s) => {
      ...
      const colCounts: Record<string, number> = {};
      Object.values(nextErrors).forEach((cellErrs) => {
  ...
  fixAll(rows) {
    ...
    // validate all rows after fixing
